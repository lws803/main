...
* Only one tag at a time can be searched eg. `find t/helloWorld` will find tags cotaining `helloWorld` and `find t/helloWorld t/world` will find tags containing `world`
****
...
===== Search guessing
This feature will also display actual keyword matches to give you a good indicator of which words are guessed.

image::find_results1.png[width]

From the above, we can see that `Marcuu` was not matched and thus it is not shown in the output for keyword(s) matched. +
Despite not matching the actual name, person `Marcus Phua Zheng Jie` will still be shown as search will attempt to guess the next closest keyword to the one you entered.

NOTE: If the keywords entered bears no similarity with any keywords among the contacts in your address book, it will list all your contacts.

Examples:

* `find p/9123` +
Displays any person having the phone number similar to `9123` eg. `91231231`, `91231234`, and so on. But it will show `0 keyword(s) matched`.
...
CAUTION: Not to be confused with a login function, this feature does not actually store your password within the address book. +
This feature is more akin to a safe in which you can lock and unlock for a one time use.
