...
Figure 3. Interactions between the `EventCenter`, `UI` and `Storage` Component for the `schedule-add d/01/01/2018 a/Complete report.` command
[NOTE]
...
The interactions between the `EventCenter`, `UI` and `Storage` Component `schedule-delete 2` command (*Step 7.*) are similar to *adding
...
The interactions between the `EventCenter`, `UI` and `Storage` Component `schedule-edit 2 a/Interview intern.`
