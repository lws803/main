...
image::scheduleAddSequenceDiagramLogic.png[width="800"]
...
image::scheduleAddSequenceDiagramStorage.png[height="800"]
...
image::scheduleAddSequenceDiagramEvent.png[height="800"]
Figure 2. Interactions between the EventCenter and the UI and Storage Component for the `schedule-add d/01/01/2018 a/Complete report.` command
[NOTE]
...
`Model#deleteActivity()` which deletes the corresponding `activity` from the `schedule`.`indicateAddressBookChanged()` is called
within `Model#deleteActivity()` to raise an `AddressBookChangedEvent` that the information within the
address book is changed.  +
Step 6. The `UI` object, `schedulePanel` which is subscribed to the event, receives this updated information and updates
the display to show the correct information to the user. +
...
`Model#updateActivity()` which updates the corresponding `activity` in the `schedule`.
