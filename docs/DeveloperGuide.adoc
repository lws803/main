...
===== Schedule
The schedule is a `TreeMap` with `Date` of existing activities as its `key` and a `list` of activities, which is due on the same
date, as its `value`. The schedule is instantiated in the `addressBook` and can be accessed through methods in the `modelManager`
