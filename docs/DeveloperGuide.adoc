...
====== Launching CorpPro:
Step 1. The user launches the application. The schedule will be set with the data read from `addressbook.xml`. +
The following sequence diagram shows you how the operation works:


====== Adding an `Activity`:

Step 1. The user executes `schedule-add d/01/01/2018 a/Complete report.` to add an activity to their schedule. After being parsed by
`addressbookParser` and `ScheduleAddCommandParser` respectively, the `ScheduleAddCommand` calls `Model#addActivity()` and the `activity`
is added to the `schedule`.

The following sequence diagram shows you how the add operation works:

====== Deleting an `Activity`:

Step 1. The user executes `schedule-delete 2` to delete the activity at `INDEX 2` in their schedule. After being parsed by
`addressbookParser` and `ScheduleDeleteCommandParser` respectively, the `ScheduleDeleteCommand` calls `Model#deleteActivity()` and the `activity`
is deleted from the `schedule`.

The following sequence diagram shows you how the delete operation works:

====== Editing an `Activity`:

Step 1. The user executes `schedule-edit 2 a/Interview intern.` to edit an activity at `INDEX 2` in their schedule. After being parsed by
`addressbookParser` and `ScheduleEditCommandParser` respectively, the `ScheduleEditCommand` calls `Model#deleteActivity()`, followed by
`Model#addActivity()` and the `activity` is edited in the `schedule`.

The following sequence diagram shows you how the edit operation works:
